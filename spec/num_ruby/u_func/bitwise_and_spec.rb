...
RSpec.describe NumRuby::UFunc::BitwiseAnd do
  describe '.call' do
    ...
    it 'performs bitwise AND operation correctly' do
      expect(result).to eq(Numo::Int32[1, 5, 7])
    end
    ...
    it 'handles all bits set correctly' do
      ...
      y_all = Numo::Int32[255, 63]
      result = described_class.call(x_all, y_all)
      expect(result).to eq(Numo::Int32[255, 63])
    end
    ...
    it 'preserves array shape' do
      ...
      expect(result).to eq(Numo::Int32[[0, 2], [4, 6]])
    end
    ...
    it 'handles negative numbers correctly' do
      ...
      # Bitwise AND should work with two's complement representation
      expect(result.class).to eq(Numo::Int32)
    end
  end

  describe '.bitwise_and' do
    subject(:result) { NumRuby.bitwise_and(x, y) }
    ...
    it 'handles zero values correctly' do
      x_zero = Numo::Int32[0, 5, 0]
      y_zero = Numo::Int32[3, 0, 7]
      result = NumRuby.bitwise_and(x_zero, y_zero)
      expect(result).to eq(Numo::Int32[0, 0, 0])
    end

    it 'demonstrates AND properties' do
      # x & x = x (idempotent)
      x_test = Numo::Int32[5, 7, 15]
      result = NumRuby.bitwise_and(x_test, x_test)
      expect(result).to eq(x_test)

      # x & 0 = 0
      zeros = Numo::Int32.zeros(3)
      result = NumRuby.bitwise_and(x_test, zeros)
      expect(result).to eq(zeros)
    end
  end
end
